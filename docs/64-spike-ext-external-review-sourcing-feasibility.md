...
A specific proposed reviewer arrangement is `INDEPENDENT` only if **all nine** of the following are true and evidenced. Any one verified false makes the arrangement `NOT_INDEPENDENT`. Any one unverifiable or undocumented — including all nine before any real arrangement exists — makes the arrangement `UNKNOWN_INDEPENDENCE`, consistent with `docs/58` section 13's "missing provenance is `UNKNOWN_INDEPENDENCE`."
...
8. Payment or engagement was structured as pay-for-service, never contingent on reaching a particular verdict or outcome.
...
Criterion 6 is the specific rule that closes the section 4.5 false-positive trap: an arrangement fails criterion 6 — and is therefore `NOT_INDEPENDENT` — the moment the reviewing account, API key, or billing is Owner-controlled, regardless of which AI vendor answered the prompt.

Criterion 9 is deliberately circular with section 6 below: this spike treats "we claim independence" and "we can produce an authenticated provenance record proving independence" as the same requirement. A claim without the record is, at best, `UNKNOWN_INDEPENDENCE`.
...
3. **Independent timestamp.** The submission channel's own timestamp (a GitHub commit timestamp, an email `Date` header with DKIM verification, a platform delivery record, or, for higher assurance, an RFC 3161 trusted-timestamp on the hashed artifact) is captured alongside the hash, so the submission time cannot be silently backdated or postdated later.
...
- **Three lanes avoid a two-way deadlock and add rough consensus signal.** Two independent lanes that disagree leave no tie-breaker; a third lane, especially from a different source class, gives the qualification campaign a rough majority signal and reduces the odds that one lane's specific blind spot goes unchallenged.
...
**Disposition: open gap. No genuinely independent external-review source is currently active or engaged for this project.**

The evidence for this, gathered directly rather than assumed:
