...
- Fresh CodeRabbit exact-head completion review of v13 candidate `f3f1f25da2f317fb7068fc22c0bc45da40e48718` against canonical base `b5dd07b6a0d2cfed42a111750c0c2df6559a0fb5` (run `f58ceec0-91ce-4e58-ada4-1139eaed736d`) found one remaining local-snapshot error-boundary defect: a non-UTF-8 POSIX symlink target could escape authority construction as `UnicodeEncodeError`, with duplicate encoding logic in tracked and untracked paths.
...
## v14 symlink-target canonical encoding repair

Fresh CodeRabbit completion review of exact v13 head `f3f1f25da2f317fb7068fc22c0bc45da40e48718` (run `f58ceec0-91ce-4e58-ada4-1139eaed736d`) found one actionable local-snapshot error-boundary defect. On POSIX, `os.readlink()` can return a surrogate-escaped string for a non-UTF-8 symlink target; the duplicated tracked/untracked `target.encode('utf-8')` calls then leaked `UnicodeEncodeError` instead of failing inside the Review World error contract.

The existing Git regression file was extended with two parameterized hostile cases—tracked and untracked malformed `b'\xff'` targets—and published RED-first on `8391ba892d7177656f3c640528fd8496ee7517a5`. CI `33853885652` produced **1255 passed / 2 historical XFAIL / 3 failed**: exactly those two runtime failures plus the deliberately stale candidate content binding.

The production repair is one shared boundary:

- `_read_symlink_target(...)` reads the target once, requires canonical UTF-8 encodability, and derives the SHA-256 digest from the exact encoded bytes;
- UTF-8 encoding failure is translated to `GitCommandError`, preserving the Review World fail-closed error contract;
- tracked and untracked snapshot construction both use the same helper, eliminating the duplicated encoding path.

The repair is frozen at intermediate head `97e9b343ba7e323070c95da9ae578ad722fe51fd`. From RED head to intermediate, production changed exactly **1 file / 16 lines** (`+11 / -5`). Exact production blob is `main_review/review_world_git.py` `d8837668ad4704e4fbe6f43cdd569a93b4f00afc`; exact regression blob is `tests/test_review_world_git.py` `b0b353a9d43d6de6872e2ef0bd230e15c5abada1`. Complete-repository intermediate CI `33854082595` produced **1257 passed / 2 historical XFAIL / exactly 1 failed**, solely the intentionally stale candidate content binding; both malformed-symlink cases are GREEN. Main Review `33854082712` passed.

## Current proof boundary

The final v8 focused SAE-10 collection was **128 tests** with a reconciled production dependency surface of **115 passed / 0 failed**. v9 added one production regression node, yielding **129 / 116**. v10 added no test node. v11 added **3 focused nodes**, two of which exercise production authority boundaries and one of which binds review evidence, yielding **132 / 118**. v12 added **5 hostile runtime cases** in the existing bound regression fixture and no new manifest-proof node, yielding **137 / 123**. v13 added exactly **4** newly collected hostile path-container cases in that same fixture and no new manifest-proof node, yielding **141 / 127**. v14 adds exactly **2** newly collected malformed-symlink hostile cases in the existing Git regression file and no new manifest-proof node, yielding structurally reconciled accounting of **143 focused tests / 129 production cases**. The final v14 full-repository CI remains the authoritative execution measurement.
