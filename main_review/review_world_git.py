...
class GitObjectResolver:
    ...
    def _run(self, *args: str) -> str:
        ...
        if completed.returncode != 0:
            ...
            raise GitCommandError(f"git {' '.join(args)} failed with exit {completed.returncode}: {detail or 'no diagnostic'}")
        return completed.stdout.strip()

    def _run_bytes(self, *args: str) -> bytes:
        try:
            completed = subprocess.run(["git", *args], cwd=self.root, capture_output=True, check=False)
        except OSError as error:
            raise GitCommandError(f"git command unavailable: {error}") from error
        if completed.returncode != 0:
...
def build_github_review_world(*, repository: str, pr_number: int, base_sha: str, head_sha: str, scope: ReviewScope, review_mode: str, rab_id: str, review_generation: str, resolver: GitObjectResolverProtocol) -> GitHubReviewWorld:
    ...
    diff = GitHubDiffIdentity.create(repository=repository, base_commit=base_sha, base_tree=base_tree, head_commit=head_sha, head_tree=head_tree, scope=scope)
    merge_tree = resolver.synthetic_merge(base_sha, head_sha) if review_mode == "merge_result" else None
    return GitHubReviewWorld.create(repository=repository, pr_number=pr_number, diff=diff, scope=scope, review_mode=review_mode, rab_id=rab_id, review_generation=review_generation, merge_tree=merge_tree)
...
def _normalize_policy_paths(paths: Sequence[str]) -> tuple[str, ...]:
    if not paths:
        return ()
    return ReviewScope.selected_paths(paths).paths
...
class LocalSnapshotPolicy:
    ...
    @classmethod
    def exclude_untracked(cls) -> "LocalSnapshotPolicy":
        return cls(untracked_policy="exclude_untracked")

    @classmethod
    def include_all_untracked_in_scope(cls) -> "LocalSnapshotPolicy":
        return cls(untracked_policy="include_all_untracked_in_scope")

    @classmethod
    def include_selected_untracked(cls, paths: Sequence[str]) -> "LocalSnapshotPolicy":
...
class LocalSnapshot:
    ...
    def to_payload(self, *, include_id: bool = True) -> dict[str, object]:
        payload: dict[str, object] = {"schema_version": self.schema_version, "head_commit": self.head_commit, "head_tree": self.head_tree, "scope": self.scope.to_payload(), "policy": self.policy.to_payload(), "entries": [entry.to_payload() for entry in self.entries], "selected_scope_digest": self.selected_scope_digest}
        if include_id:
            payload["local_snapshot_id"] = self.local_snapshot_id
        return payload
...
def _decode_git_path(raw: bytes) -> str:
    try:
        ...
        raise GitCommandError("local Review World cannot canonically encode non-UTF-8 Git path") from error
...
def _path_in_scope(path: str, scope: ReviewScope) -> bool:
    if scope.kind == "repository":
        return True
    return any(path == selected or path.startswith(selected.rstrip("/") + "/") for selected in scope.paths)
...
def _index_entries(resolver: GitObjectResolver, scope: ReviewScope) -> list[tuple[str, str, str]]:
    ...
    raw = resolver._run_bytes("ls-files", "-s", "-z")
    for record in raw.split(b"\0"):
        ...
        except (ValueError, UnicodeDecodeError, ReviewWorldError) as error:
            raise GitCommandError("git index entry is malformed or ambiguous") from error
        if stage != "0":
...
def _submodule_state(root: Path, path: str, index_oid: str, policy: LocalSnapshotPolicy) -> LocalPathState:
    ...
    except ReviewWorldError as error:
        raise GitCommandError(f"submodule material unresolved for {path}: {error}") from error
...
def build_local_snapshot(root: str | Path, *, scope: ReviewScope, policy: LocalSnapshotPolicy) -> LocalSnapshot:
    ...
    scope_payload: dict[str, object] = {"scope_id": scope.scope_id, "policy": policy.to_payload(), "entries": [entry.to_payload() for entry in states]}
    selected_scope_digest = sha256_id(scope_payload)
    body: dict[str, object] = {"schema_version": "sergeant.local-snapshot.v1", "head_commit": head_commit, "head_tree": head_tree, "scope": scope.to_payload(), "policy": policy.to_payload(), "entries": [entry.to_payload() for entry in states], "selected_scope_digest": selected_scope_digest}
