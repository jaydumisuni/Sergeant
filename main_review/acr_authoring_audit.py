...
from .assurance_contract_registry import (
    ACRContract, ApplicabilityPredicate, CardinalityKind, ClosureGrade,
    CollectionRequirement, ContractRequirement, NegativeApplicabilityBurden,
    RegistryError,
)
...
class AuthoringAuditProfile:
    ...
    @classmethod
    def create(cls,*,profile_id,generation,contract_id,contract_generation,domain_id,domain_hash,expected_applicability,
        ...
        contract_id=_string(contract_id,"contract_id"); contract_generation=_string(contract_generation,"contract_generation")
        domain_id=_string(domain_id,"domain_id"); domain_hash=require_full_sha256(domain_hash,"domain_hash")
        ...
        if facts!=expected_applicability.referenced_facts(): raise ValueError("required_applicability_facts must exactly match expected applicability references")
        ...
        collections=tuple(sorted(tuple(required_collections),key=lambda x:x.family))
        ...
        body={"schema_version":"sergeant.acr-authoring-audit-profile.v3","profile_id":profile_id,"generation":generation,
              "contract_id":contract_id,"contract_generation":contract_generation,"domain_id":domain_id,"domain_hash":domain_hash,
              "expected_applicability":expected_applicability.to_payload(),"independent_basis_ids":list(bases),"required_applicability_facts":list(facts),
              "required_bound_subject_variables":list(vals["required_bound_subject_variables"]),"required_semantic_carriers":list(vals["required_semantic_carriers"]),
              "required_consumer_interpretation_families":list(vals["required_consumer_interpretation_families"]),"required_affected_relations":list(vals["required_affected_relations"]),
              "required_collections":[x.to_payload() for x in collections],"required_premises":[x.to_payload() for x in vals["required_premises"]],
              "required_repeated_authority_premise_families":list(vals["required_repeated_authority_premise_families"]),"required_obligations":[x.to_payload() for x in vals["required_obligations"]],
              "required_admissible_proof_classes":list(vals["required_admissible_proof_classes"]),"required_material_inputs":[x.to_payload() for x in vals["required_material_inputs"]],
              "required_coherence_rules":list(vals["required_coherence_rules"]),"required_temporal_rules":list(vals["required_temporal_rules"]),
              "required_falsifier_families":list(vals["required_falsifier_families"]),"required_independence":list(vals["required_independence"]),
              "required_permitted_capabilities":list(vals["required_permitted_capabilities"]),"required_external_review_lanes":{k:v for k,v in lanes},
              "require_negative_applicability_burden":True,"require_unknown_fallback":True}
        return cls(body["schema_version"],profile_id,generation,contract_id,contract_generation,domain_id,domain_hash,expected_applicability,bases,facts,
    ...
    def to_payload(self):
        return {"schema_version":self.schema_version,"profile_id":self.profile_id,"generation":self.generation,"contract_id":self.contract_id,"contract_generation":self.contract_generation,
                "domain_id":self.domain_id,"domain_hash":self.domain_hash,"expected_applicability":self.expected_applicability.to_payload(),"independent_basis_ids":list(self.independent_basis_ids),
                "required_applicability_facts":list(self.required_applicability_facts),"required_bound_subject_variables":list(self.required_bound_subject_variables),
                "required_semantic_carriers":list(self.required_semantic_carriers),"required_consumer_interpretation_families":list(self.required_consumer_interpretation_families),
                "required_affected_relations":list(self.required_affected_relations),"required_collections":[x.to_payload() for x in self.required_collections],
                "required_premises":[x.to_payload() for x in self.required_premises],"required_repeated_authority_premise_families":list(self.required_repeated_authority_premise_families),
                "required_obligations":[x.to_payload() for x in self.required_obligations],"required_admissible_proof_classes":list(self.required_admissible_proof_classes),
                "required_material_inputs":[x.to_payload() for x in self.required_material_inputs],"required_coherence_rules":list(self.required_coherence_rules),
                "required_temporal_rules":list(self.required_temporal_rules),"required_falsifier_families":list(self.required_falsifier_families),
                "required_independence":list(self.required_independence),"required_permitted_capabilities":list(self.required_permitted_capabilities),
                "required_external_review_lanes":{k:v for k,v in self.required_external_review_lanes},"require_negative_applicability_burden":self.require_negative_applicability_burden,
                "require_unknown_fallback":self.require_unknown_fallback,"profile_hash":self.profile_hash}
    @classmethod
    def from_payload(cls,p):
        expected=set(cls.__dataclass_fields__)
        if set(p)!=expected: raise ValueError("AuthoringAuditProfile persisted payload has wrong fields")
        if p["schema_version"]!="sergeant.acr-authoring-audit-profile.v3": raise ValueError("unknown AuthoringAuditProfile schema")
        ...
        obj=cls.create(profile_id=p["profile_id"],generation=p["generation"],contract_id=p["contract_id"],contract_generation=p["contract_generation"],domain_id=p["domain_id"],domain_hash=p["domain_hash"],
                       expected_applicability=ApplicabilityPredicate.from_payload(p["expected_applicability"]),independent_basis_ids=p["independent_basis_ids"],required_applicability_facts=p["required_applicability_facts"],
                       required_bound_subject_variables=p["required_bound_subject_variables"],required_semantic_carriers=p["required_semantic_carriers"],required_consumer_interpretation_families=p["required_consumer_interpretation_families"],
...
def audit_contract_authoring(contract: ACRContract,profile: AuthoringAuditProfile)->AuthoringAuditResult:
    ...
    findings=[]
    try:
        AuthoringAuditProfile.from_payload(profile.to_payload())
    except Exception as exc:
        _finding(findings,"profile_noncanonical_or_malformed",f"authoring profile failed canonical validation: {type(exc).__name__}")
        return AuthoringAuditResult(AuthoringAuditStatus.DEFICIENT,tuple(findings),False)
    canonical=True
    ...
    if not canonical:
        negative_ok=False
        try:
            if isinstance(contract.negative_applicability,NegativeApplicabilityBurden): contract.negative_applicability.validate(); negative_ok=True
        except (RegistryError,AttributeError,TypeError): pass
        if profile.require_negative_applicability_burden and not negative_ok: _finding(findings,"negative_applicability_burden_missing","canonical PROVEN_NO_MATCH burden with sufficient closure missing")
        if getattr(contract,"unsupported_fallback",None)!="UNKNOWN": _finding(findings,"unknown_fallback_weakening","unsupported fallback must be UNKNOWN")
        if getattr(contract,"self_qualification_allowed",None) is not False: _finding(findings,"candidate_self_qualification","candidate contract attempted to grant qualification")
        ordered=tuple(sorted(set(findings),key=lambda x:(x.family,x.detail))); return AuthoringAuditResult(AuthoringAuditStatus.DEFICIENT,ordered,False)
    if contract.contract_id!=profile.contract_id or contract.generation!=profile.contract_generation or contract.domain.domain_id!=profile.domain_id or contract.domain.domain_hash!=profile.domain_hash:
        _finding(findings,"audit_scope_mismatch","contract/profile identity, generation, or exact bounded-domain identity mismatch")
    if contract.applicability.to_payload()!=profile.expected_applicability.to_payload(): _finding(findings,"applicability_semantics_mismatch","applicability operator/value/tree differs from independently audited predicate")
    refs=set(contract.applicability.referenced_facts()); missing=set(profile.required_applicability_facts)-refs
    if missing: _finding(findings,"applicability_omission",f"missing applicability facts: {sorted(missing)!r}")
    for family,expected,actual in (("bound_subject_variables_mismatch",profile.required_bound_subject_variables,contract.bound_subject_variables),("admissible_proof_classes_mismatch",profile.required_admissible_proof_classes,contract.admissible_proof_classes),("permitted_capabilities_mismatch",profile.required_permitted_capabilities,contract.permitted_capabilities)):
    ...
    for family,expected,actual in (("semantic_carrier_omission",set(profile.required_semantic_carriers),set(contract.semantic_carrier_families)),("consumer_interpretation_omission",set(profile.required_consumer_interpretation_families),set(contract.consumer_interpretation_families)),("affected_relation_omission",set(profile.required_affected_relations),set(contract.affected_relation_families)),("repeated_authority_premise_omission",set(profile.required_repeated_authority_premise_families),set(contract.repeated_authority_premise_families)),("coherence_rule_omission",set(profile.required_coherence_rules),set(contract.coherence_rules)),("temporal_rule_omission",set(profile.required_temporal_rules),set(contract.temporal_rules)),("falsifier_family_omission",set(profile.required_falsifier_families),set(contract.mandatory_falsifier_families)),("independence_rule_omission",set(profile.required_independence),set(contract.required_independence))):
        absent=expected-actual
        if absent: _finding(findings,family,f"missing required families/rules: {sorted(absent)!r}")
    _audit_requirement_set(findings,contract.mandatory_premises,profile.required_premises,"premise"); _audit_requirement_set(findings,contract.mandatory_obligations,profile.required_obligations,"obligation"); _audit_requirement_set(findings,contract.material_inputs,profile.required_material_inputs,"material_input")
    ...
    for required in profile.required_collections:
        ...
        if actual.semantics is not required.semantics or not _cardinality_at_least(actual.cardinality,required.cardinality): _finding(findings,"collection_semantics_or_cardinality_weakening",f"weakened collection {required.family}")
        if _closure_rank(actual.required_closure)<_closure_rank(required.required_closure): _finding(findings,"closure_grade_weakening",f"weakened closure for {required.family}")
    ...
    negative_ok=False
    try: contract.negative_applicability.validate(); negative_ok=True
    ...
    ordered=tuple(sorted(set(findings),key=lambda x:(x.family,x.detail))); return AuthoringAuditResult(AuthoringAuditStatus.DEFICIENT if ordered else AuthoringAuditStatus.CLEAN,ordered,False)
...
class ACRQualificationEscapeRecord:
    ...
    def to_payload(self): return {"schema_version":self.schema_version,"registry_id":self.registry_id,"contract_id":self.contract_id,"escaped_generation":self.escaped_generation,"defect_family":self.defect_family,"evidence_ids":list(self.evidence_ids),"disposition":self.disposition.value,"impact_analysis_required":self.impact_analysis_required,"automatic_corrected_contract_promotion_allowed":self.automatic_corrected_contract_promotion_allowed,"permanent_qualification_evidence":self.permanent_qualification_evidence,"escape_id":self.escape_id}
    @classmethod
    def from_payload(cls,p):
        ...
        if p["disposition"]!=ACREscapeDisposition.SUSPEND_OR_REVOKE.value or p["impact_analysis_required"] is not True or p["automatic_corrected_contract_promotion_allowed"] is not False or p["permanent_qualification_evidence"] is not True: raise ValueError("invalid qualification escape authority state")
        ...
        if require_full_sha256(p["escape_id"],"escape_id")!=obj.escape_id or obj.to_payload()!=p: raise ValueError("ACR qualification escape payload is non-canonical")
        return obj
