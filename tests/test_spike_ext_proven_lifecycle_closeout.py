...
def test_spike_ext_closeout_binds_reviewed_candidate_artifacts_without_full_history() -> None:
    closeout = _load(CLOSEOUT_MANIFEST_PATH)
    historical = closeout["historical_candidate"]
    reconciliation = closeout["reconciliation"]

    assert historical["pull_request"] == 171
    assert len(historical["head"]) == 40
    int(historical["head"], 16)
    assert len(reconciliation["transplant_commit"]) == 40
    int(reconciliation["transplant_commit"], 16)

    # The current branch carries the exact reviewed candidate artifacts.
    # This proof deliberately does not require historical commits to exist in
    # the local clone because Sergeant's normal CI checkout is depth-1.
    ...
    assert reconciliation["local_history_required_for_proof"] is False
    assert reconciliation["github_lineage_requires_live_recovery"] is True
