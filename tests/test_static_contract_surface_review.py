from __future__ import annotations

from pathlib import Path

from main_review.static_contract_surface_review import run_static_contract_surface_review
from main_review.static_status_review import run_static_status_review


UI_ROOT = "ui-save-clears-edit-without-durable-persistence"
SHAPE_ROOT = "malformed-collection-response-silently-treated-as-empty"
AUTHORITY_ROOT = "process-wide-mutable-runtime-authority-in-command-registry"


def _roots(result: dict) -> set[str]:
    return {str(item.get("root_cause")) for item in result.get("findings", [])}


def test_state_only_ui_save_that_exits_edit_mode_is_reported(tmp_path: Path) -> None:
    source = tmp_path / "PropertyTab.tsx"
    source.write_text(
        """
const handlePlanEditSave = () => {
  if (planEditDraft) {
    setFloorPlans(prev =>
      prev.map(plan => plan.id === planEditDraft.id ? planEditDraft : plan)
    );
    setEditingPlanId(null);
    setPlanEditDraft(null);
  }
};

const handleEditSave = () => {
  if (editDraft) {
    setUnits(prev => prev.map(unit => unit.id === editDraft.id ? editDraft : unit));
    setEditingUnitId(null);
    setEditDraft(null);
  }
};
        """,
        encoding="utf-8",
    )

    result = run_static_contract_surface_review(tmp_path, ["PropertyTab.tsx"])

    findings = [item for item in result["findings"] if item["root_cause"] == UI_ROOT]
    assert len(findings) == 2
    assert all(item["severity"] == "major" for item in findings)


def test_persisted_ui_save_that_stays_open_on_failure_is_clean(tmp_path: Path) -> None:
    source = tmp_path / "PropertyTab.tsx"
    source.write_text(
        """
const handlePlanEditSave = async () => {
  if (!planEditDraft) return;
  const response = await fetch(`/api/plans/${planEditDraft.id}`, {
    method: "PATCH",
    body: JSON.stringify(planEditDraft),
  });
  if (!response.ok) {
    setSaveError(await response.text());
    return;
  }
  setFloorPlans(prev =>
    prev.map(plan => plan.id === planEditDraft.id ? planEditDraft : plan)
  );
  setEditingPlanId(null);
  setPlanEditDraft(null);
};
        """,
        encoding="utf-8",
    )

    result = run_static_contract_surface_review(tmp_path, ["PropertyTab.tsx"])

    assert UI_ROOT not in _roots(result)


def test_editor_state_only_cancel_is_not_misclassified_as_save(tmp_path: Path) -> None:
    source = tmp_path / "PropertyTab.tsx"
    source.write_text(
        """
const handlePlanEditCancel = () => {
  setEditingPlanId(null);
  setPlanEditDraft(null);
};
        """,
        encoding="utf-8",
    )

    result = run_static_contract_surface_review(tmp_path, ["PropertyTab.tsx"])

    assert UI_ROOT not in _roots(result)


def test_dart_collection_contract_mismatch_returning_empty_is_reported(tmp_path: Path) -> None:
    source = tmp_path / "trip_service.dart"
    source.write_text(
        """
class TripService {
  Future<List<Map<String, dynamic>>> fetchTripItems(String tripDisplayId) async {
    final body = await _apiClient.get('/api/trips/$tripDisplayId/items');
    if (body is! List) return [];
    return List<Map<String, dynamic>>.from(body);
  }
}
        """,
        encoding="utf-8",
    )

    result = run_static_contract_surface_review(tmp_path, ["trip_service.dart"])

    assert SHAPE_ROOT in _roots(result)


def test_dart_collection_contract_mismatch_throwing_typed_error_is_clean(tmp_path: Path) -> None:
    source = tmp_path / "trip_service.dart"
    source.write_text(
        """
class TripService {
  Future<List<Map<String, dynamic>>> fetchTripItems(String tripDisplayId) async {
    final body = await _apiClient.get('/api/trips/$tripDisplayId/items');
    if (body is! List) {
      throw StateError('Unexpected trip items response type');
    }
    return List<Map<String, dynamic>>.from(body);
  }
}
        """,
        encoding="utf-8",
    )

    result = run_static_contract_surface_review(tmp_path, ["trip_service.dart"])

    assert SHAPE_ROOT not in _roots(result)


def test_command_registry_process_global_runtime_authority_is_reported(tmp_path: Path) -> None:
    header = tmp_path / "CommandRegistry.h"
    header.write_text(
        """
#pragma once
#include <functional>

class AudioEngine;
class CommandRegistry {
public:
  using Factory = std::function<int(const Flags&)>;
  static void setAudioEngine(AudioEngine* engine);
  static AudioEngine* getAudioEngine();
  void registerCommand(const char* name, Factory factory);
};
        """,
        encoding="utf-8",
    )
    implementation = tmp_path / "CommandRegistry.cpp"
    implementation.write_text(
        """
#include "CommandRegistry.h"

namespace {
AudioEngine* s_audioEngine = nullptr;
}

void CommandRegistry::setAudioEngine(AudioEngine* engine) {
  s_audioEngine = engine;
}

AudioEngine* CommandRegistry::getAudioEngine() {
  return s_audioEngine;
}

void CommandRegistry::initialize() {
  reg.registerCommand("play", [](const auto&) {
    AudioEngine* engine = getAudioEngine();
    return engine ? 1 : 0;
  });
}
        """,
        encoding="utf-8",
    )

    result = run_static_contract_surface_review(
        tmp_path,
        ["CommandRegistry.h", "CommandRegistry.cpp"],
    )

    assert AUTHORITY_ROOT in _roots(result)


def test_explicit_per_build_context_is_clean(tmp_path: Path) -> None:
    header = tmp_path / "CommandRegistry.h"
    header.write_text(
        """
#pragma once
#include <functional>

struct CommandContext {
  AudioEngine* engine;
  TrackManager* trackManager;
};

class CommandRegistry {
public:
  using Factory = std::function<int(const Flags&, const CommandContext&)>;
  int build(const Flags& flags, const CommandContext& context);
};
        """,
        encoding="utf-8",
    )
    implementation = tmp_path / "CommandRegistry.cpp"
    implementation.write_text(
        """
#include "CommandRegistry.h"

void CommandRegistry::initialize() {
  reg.registerCommand("play", [](const auto&, const CommandContext& context) {
    return context.engine ? 1 : 0;
  });
}
        """,
        encoding="utf-8",
    )

    result = run_static_contract_surface_review(
        tmp_path,
        ["CommandRegistry.h", "CommandRegistry.cpp"],
    )

    assert AUTHORITY_ROOT not in _roots(result)


def test_unrelated_static_constant_in_registry_is_clean(tmp_path: Path) -> None:
    source = tmp_path / "CommandRegistry.cpp"
    source.write_text(
        """
namespace {
constexpr int kMaxCommands = 64;
}
void CommandRegistry::registerDefaults() {
  registerCommand("noop", [](const auto&) { return kMaxCommands; });
}
        """,
        encoding="utf-8",
    )

    result = run_static_contract_surface_review(tmp_path, ["CommandRegistry.cpp"])

    assert not result["findings"]


def test_status_bundle_exposes_all_contract_surface_roots(tmp_path: Path) -> None:
    ui = tmp_path / "PropertyTab.tsx"
    ui.write_text(
        """
const handleEditSave = () => {
  setUnits(prev => prev.map(unit => unit.id === editDraft.id ? editDraft : unit));
  setEditingUnitId(null);
  setEditDraft(null);
};
        """,
        encoding="utf-8",
    )
    dart = tmp_path / "trip_service.dart"
    dart.write_text(
        """
Future<List<Map<String, dynamic>>> fetchTripItems(String id) async {
  final body = await _apiClient.get('/items');
  if (body is! List) return [];
  return List<Map<String, dynamic>>.from(body);
}
        """,
        encoding="utf-8",
    )
    header = tmp_path / "CommandRegistry.h"
    header.write_text(
        """
class CommandRegistry {
public:
  using Factory = int;
  static void setAudioEngine(AudioEngine* engine);
  static AudioEngine* getAudioEngine();
};
        """,
        encoding="utf-8",
    )
    implementation = tmp_path / "CommandRegistry.cpp"
    implementation.write_text(
        """
AudioEngine* s_audioEngine = nullptr;
void CommandRegistry::setAudioEngine(AudioEngine* engine) { s_audioEngine = engine; }
AudioEngine* CommandRegistry::getAudioEngine() { return s_audioEngine; }
void CommandRegistry::initialize() {
  reg.registerCommand("play", [] { return getAudioEngine() != nullptr; });
}
        """,
        encoding="utf-8",
    )

    result = run_static_status_review(
        tmp_path,
        [
            "PropertyTab.tsx",
            "trip_service.dart",
            "CommandRegistry.h",
            "CommandRegistry.cpp",
        ],
    )

    assert {UI_ROOT, SHAPE_ROOT, AUTHORITY_ROOT}.issubset(_roots(result))
    assert result["static_contract_surface_review"]["finding_count"] == 3
