...
def test_corrected_durable_visual_copy_remains_fail_closed_until_replayed() -> None:
    text = VISUAL_PROOF.read_text(encoding="utf-8")
    if "Corrected durable visual copy: PASS" not in text:
        assert "Corrected durable visual copy: PENDING" in text
        assert "Corrected recovery byte replay: PENDING" in text
