...
def contract(
    ...
    *,
    ...
    proof_classes: tuple[str, ...] = ("mechanical", "exhaustive-oracle", "heuristic"),
) -> ACRContract:
...
def qualified_fixture(*, overlapping: bool = False):
    first = contract("primary-authz")
    contracts = [first]
    if overlapping:
        contracts.append(
            contract(
    ...
    return qualified, closure.expected_obligations[0], by_id
...
def test_overlapping_origins_require_conservative_union_of_material_inputs() -> None:
    qualified, obligation, contracts = qualified_fixture(overlapping=True)
    world = coordinates()
    complete = evidence(
        obligation,
        world,
