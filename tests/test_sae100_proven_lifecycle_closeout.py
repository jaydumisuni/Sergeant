...
def test_candidate_authority_blobs_are_frozen():
    ...
    for path, key in expected:
        assert blob_at(g["head"], path) == g[key]
...
def test_closeout_binds_qualified_dependency_repair_identity():
    repair = load()["qualified_dependency_repair"]
    assert repair["pull_request"] == 227
    assert repair["head"] == "0fc3efb48ee04b76270619607cb577d32e20962c"
    assert repair["canonical_merge"] == "66601d2854073d9ff8ebe14f0370d8221a73ed3e"
    assert repair["full_suite"] == "1669 passed, 2 xfailed"
    assert repair["independent_review"]["verdict"] == "APPROVE"
    assert repair["independent_review"]["confidence"] == 0.88
    assert repair["independent_review"]["required_actions"] == []
    assert repair["qualified_dependency_enforcement"] is True
    assert repair["qualified_rust_kernel_enforcement"] is True
